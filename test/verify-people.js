...
const DIR = {
  ...
  'julian.marsh@example.com':       { person_id: 'p_gamal',       role: 'employee', manager_email: 'adam.foster@example.com' },
...
(async () => {
  try {
    ...
    assert(byId('p_akram').name === 'Adam Foster', 'A: mock fields intact when signed out');
    ...
    WP._sb = makeFakeSb([ row('p_akram', { name: 'Adam Foster (server)', title: 'Promoted Title', level: 'sr_manager', manager_id: 'p_motaa', initials: 'MA' }) ]);
    resetDb();
    res = await WP.db.people.list();
    const akramAfter = byId('p_akram');
    assert(akramAfter.name === 'Adam Foster (server)' && akramAfter.title === 'Promoted Title' && akramAfter.level === 'sr_manager', 'B: server wins for directory fields (name/title/level)');
    ...
    assert(byId('p_akram').name === 'Adam Foster', 'C: mock fields intact after a failed read (nothing clobbered)');
    ...
    restoreMock(); WP._sb = makeScopedSb(tag(scopedRows()), 'mason.hughes@example.com'); resetDb();
    await WP.db.people.list();
    ...
    restoreMock(); WP._sb = makeScopedSb(tag(scopedRows()), 'aaron.maxwell@example.com'); resetDb();
    await WP.db.people.list();
    ...
    restoreMock(); WP._sb = makeScopedSb(tag(scopedRows()), 'oliver.grant@example.com'); resetDb();
    await WP.db.people.list();

...
(function (WP) {
  ...
  // on the sign-in screen. The personal credit names are part of the DEMO seed — in
  // no-demo mode we must not bake real personal names into the bundle (a name-free empty
  // state + the "no real names in the bundle" rule), so we fall back to a neutral
  // department line. Names are proper nouns, kept verbatim in both languages.
      ...
      host.innerHTML =
        '<div class="sig-bar">' +
          ...
          '<div class="sig-credit">' +
            '<div class="sig-name sig-name--accent">' + t('sigOps') + '</div>' +
          '</div>' +
        '</div>';
      return;
    }

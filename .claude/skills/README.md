...
| [`tempo-bulletproof-5step`](tempo-bulletproof-5step/SKILL.md) | Any feature/bugfix that must be resilient — the Deep Dive → Implement → Chaos Test → Root Cause → Bulletproof Fix loop: build it, try to break it, fix the cause, leave a regression test so it can't come back. | Internal — scientific method, Chaos Engineering, 5-Whys, test pyramid, Constitution |
| [`tempo-preview-verify`](tempo-preview-verify/SKILL.md) | Verifying a UI change in the in-app preview browser + local dev server (pre-merge) — the gotchas that waste cycles: server dying / port collisions, geometry reads that lie when the pane is backgrounded, forced-render contamination, and overflow/NaN/dead-space/cascade recipes. | Internal — ESCAPE-LEARNING, harness tool contracts |
